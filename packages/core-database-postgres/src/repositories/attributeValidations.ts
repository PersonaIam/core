...
export class AttributeValidationsRepository extends Repository {
    ...
    public async addAttributeValidationRequestAction(parameters) {
        const query =
            `INSERT INTO attribute_validation_request_actions (attribute_validation_request_id,timestamp,action) VALUES (` +
            parameters.id +
            "," +
    ...
    public async getAttributeValidationRequests(filter) {
        ...
        if (filter.attributeId && !filter.validator) {
            query = QueriesValidation.getAttributeValidationRequestsForAttribute.replace(
                "${attributeId}",
                filter.attributeId,
            );
        } else {
            if (filter.attributeId && filter.validator) {
                ...
                if (!filter.validator && filter.owner) {
                    query = QueriesValidation.getAttributeValidationRequestsForOwner.replace(
                        "${owner}",
                        "'" + filter.owner + "'",
                    );
                }
                if (filter.validator && !filter.owner) {
                    query = QueriesValidation.getAttributeValidationRequestsForValidator.replace(
                        "${validator}",
                        "'" + filter.validator + "'",
                    );
                }
