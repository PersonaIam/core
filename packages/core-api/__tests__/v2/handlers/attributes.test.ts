        ...
        describe.each([["Accept", "requestWithAcceptHeader"]])(
            ...
            (header, request) => {
                it("As a PUBLIC user, I want to Get the details of an attribute type (FIRST_NAME)." +
                    "EXPECTED : SUCCESS. RESULT : Attribute type details (name is FIRST_NAME)", async () => {
                ...
                it("As a PUBLIC user, I want to Get the details of an attribute type that does not exist. " +
                    "EXPECTED : FAILURE. ERROR : ATTRIBUTE_TYPE_NOT_FOUND", async () => {
                ...
                it("As a PUBLIC user, I want to Get the list of attribute types. " +
                    "EXPECTED : SUCCESS. RESULT : Attribute type list", async () => {
        ...
        describe.each([["Accept", "requestWithAcceptHeader"]])(
            ...
            (header, request) => {
                it("As a PUBLIC user, I want to Get the attributes of a user (OTHER_OWNER) that has no attributes. " +
                    "EXPECTED : SUCCESS. RESULT : Empty \"attributes\" array", async () => {
                ...
                it("As an OWNER, I want to Create a non file attribute and provide associations. " +
                    "EXPECTED : FAILURE. ERROR : ASSOCIATIONS_NOT_SUPPORTED_FOR_NON_FILE_TYPES", async () => {

                    let params = <any>{};
                    params.associations = [1];
                    let body = createAttributeBody(params);
                ...
                it("Create a non file attribute (ADDRESS). " +
                    "EXPECTED : SUCCESS. RESULT : Transaction ID", async () => {
                    let param = <any>{};
                ...
                it("As a PUBLIC user, I want to Get the details of an attribute without providing the owner parameter. " +
                    "EXPECTED : FAILURE. ERROR : MISSING_OWNER_ADDRESS", async () => {
                ...
                it('As a PUBLIC user, I want to Get the details of an attribute by providing a correct owner and an incorrect attribute type. ' +
                    'EXPECTED : SUCCESS. RESULT : "attributes" as an empty array', async () => {
                ...
                it("As a PUBLIC user, I want to Get the details of an attribute that does not exist (OWNER, SSN). " +
                    "EXPECTED : SUCCESS. RESULT : Empty \"attributes\" array", async () => {
                ...
                it('As an OTHER_OWNER, I want to Create an attribute for some other user (OWNER). ' +
                    'EXPECTED : FAILURE. ERROR : SENDER_IS_NOT_OWNER', async () => {
                ...
                it('As an OWNER, I want to Create a file type attribute and provide as association an attribute that does not exist. ' +
                    'EXPECTED : FAILURE. ERROR : ATTRIBUTE_ASSOCIATION_DOES_NOT_EXIST_FOR_CURRENT_OWNER', async () => {
                ...
                it('As an OWNER, I want to Create a file type attribute and provide as association an attribute that belongs to a different user. ' +
                    'EXPECTED : FAILURE. ERROR : ATTRIBUTE_ASSOCIATION_DOES_NOT_EXIST_FOR_CURRENT_OWNER', async () => {

                    const otherOwnerAttribute = await utils[request]("GET", "v2/attributes?owner=" + OTHER_OWNER + "&type=" + FIRST_NAME);
                ...
                it('As an OWNER, I want to Create a file type attribute and provide as association 2 attributes, one of which belongs to a different user. ' +
                    'EXPECTED : FAILURE. ERROR : ATTRIBUTE_ASSOCIATION_DOES_NOT_EXIST_FOR_CURRENT_OWNER', async () => {

                    const otherOwnerAttribute = await utils[request]("GET", "v2/attributes?owner=" + OTHER_OWNER + "&type=" + FIRST_NAME);
                    const otherOwnerAttributeId = otherOwnerAttribute.data.attributes[0].id;

                    const ownerAttribute = await utils[request]("GET", "v2/attributes?owner=" + OWNER + "&type=" + PHONE_NUMBER);
                ...
                it("As a PUBLIC user, I want to Get the attributes of a user (OTHER_OWNER) that has 1 attribute. " +
                    'EXPECTED : SUCCESS. RESULT : Contains "attributes" as an array with 1 element', async () => {
                ...
                it("As a PUBLIC user, I want to Get the attributes of a user (OWNER) that has multiple attributes. " +
                    'EXPECTED : SUCCESS. RESULT : Contains "attributes" as an array with 4 elements', async () => {
                    const response = await utils[request]("GET", "v2/attributes?owner=" + OWNER);
    ...
    describe("Update Attribute", () => {
        describe.each([["Accept", "requestWithAcceptHeader"]])(
            ...
            (header, request) => {
                ...
                it("Get the attributes of a user (OWNER) that has multiple attributes, immediately after updating one of them. " +
                    'EXPECTED : SUCCESS. RESULT : Contains "attributes" as an array with 4 elements', async () => {
                    const response = await utils[request]("GET", "v2/attributes?owner=" + OWNER);
                ...
                it('As a PUBLIC user, I want to Get the details of a recently updated non file attribute (OWNER, ADDRESS). ' +
                    'EXPECTED : SUCCESS. RESULT : Contains "value" as the new address value', async () => {
                ...
                it("As an OWNER, I want to Update an attribute that does not exist. " +
                    "EXPECTED : FAILURE. ERROR : ATTRIBUTE_NOT_FOUND_FOR_UPDATE", async () => {
                ...
                it('Get the details of a recently updated non file attribute (OWNER, ADDRESS). ' +
                    'EXPECTED : SUCCESS. RESULT : Contains "expire_timestamp" as the new expire timestamp value', async () => {
                ...
                it('As an OWNER, I want to Update an attribute for some other user (OTHER_OWNER). ' +
                    'EXPECTED : FAILURE. ERROR : SENDER_IS_NOT_OWNER', async () => {

                    const attribute = await utils[request]("GET", "v2/attributes?owner=" + OTHER_OWNER + "&type=" + FIRST_NAME);
                    let param = <any>{};
                    ...
                    let body = updateAttributeRequest(param);
                    const response = await utils[request]("PUT", "v2/attributes", body);
                    expect(response.data).toHaveProperty(SUCCESS);
                    expect(response.data.success).toBe(FALSE);
                    expect(response.data).toHaveProperty(ERROR);
                ...
                it('As an OWNER, I want to Update a non file attribute (ADDRESS) but change nothing in the attribute itself. ' +
                    'EXPECTED : SUCCESS. RESULT : "message" result with value NOTHING_TO_UPDATE', async () => {
