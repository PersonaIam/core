...
const getService = async request => {
    if (
        ...
        !Crypto.Validations.validateAddress(request.query.provider, Managers.configManager.get("pubKeyHash"))
    ) {
...
const getAttributeTypes = async request => {
    ...
    if (!Crypto.Validations.validateAddress(request.query.provider, Managers.configManager.get("pubKeyHash"))) {
        return { error: messages.INVALID_PROVIDER_ADDRESS, success: false };
    }
