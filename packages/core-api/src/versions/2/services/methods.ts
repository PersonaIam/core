...
const postService = async request => {
    ...
    let postResponse = await servicesRepository.postService(request.payload);
...
const updateServiceStatus = async (request, newStatus) => {
    ...
    if (newStatus === constants.serviceStatus.INACTIVE && services.rows[0].status === constants.serviceStatus.INACTIVE) {
    ...
    let responseObject = <any>{};
    let response = await servicesRepository.putService(request.payload, newStatus);
