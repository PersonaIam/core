...
const getIdentityUseRequest = async request => {
    ...
    if (!request.query.serviceProvider && !request.query.serviceId &&
        !(request.query.serviceName && request.query.serviceProvider) && !request.query.attributeId && !request.query.owner) {
    ...
    let identityUseRequestsResult = identityUseRequests[0].map(
        identityUseRequest => transformIdentityUseRequest(identityUseRequest));

    if (identityUseRequestsResult.length > 0 &&  request.query.serviceId && request.query.owner) { // only get validation requests if there is a single IdentityUseRequest to retrieve
        let identityUseRequestsWithValidationDetails = await identityUsesRepository.getIdentityUseRequestWithValidationDetails(
            ...
            let attributeTypes = identityUseRequests[0][0].attributeTypes;
...
const createIdentityUseRequest = async (request) => {
    ...
    ownerAttributes = await extractAttributeDetails(ownerAttributes.rows, {owner : request.payload.asset.identityuse[0].owner});
    let serviceAttributes = JSON.parse(services.rows[0].attributeTypes);
    ...
    let ownerAttributesActiveNotExpiredWithValidations = ownerAttributes.filter(attribute =>

        attribute.active &&                                                                 // attribute has to be active
        (!attribute.expireTimestamp || attribute.expireTimestamp > slots.getTime()) &&    // attribute has to not be expired
        attribute.completed >= services.rows[0].validationsRequired );            // attribute has to have minimum required validations

    let ownerAttributesToCheck = ownerAttributesActiveNotExpiredWithValidations.map(attribute => attribute.type);

    let ownerAttributeTypes = ownerAttributes.map(attribute => attribute.type);

    let identityRequestAttributes = request.payload.asset.identityuse[0].attributes;
    ...
    let identityUseRequest = await identityUsesRepository.getIdentityUseRequest({
    ...
    let putResponse = await identityUsesRepository.createIdentityUseRequest(request.payload);
...
const identityUseRequestAnswer = async (request, action) => {
    ...
    let services = await servicesRepository.search({
        name : request.payload.asset.identityuse[0].serviceName,
        provider : request.payload.asset.identityuse[0].serviceProvider,
    });
    ...
    let identityUseAnswerError = checkIdentityUseAnswerForErrors({status : identityUseRequest[0][0].status, answer : action });
