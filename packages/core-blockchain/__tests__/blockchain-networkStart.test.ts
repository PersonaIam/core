...
describe("constructor - networkStart", () => {
    ...
    it("should output log messages if launched in networkStart mode", async () => {
        ...
        expect(loggerWarn).toHaveBeenCalledWith(
            "Persona Core is launched in Genesis Start mode. This is usually for starting the first node on the blockchain. Unless you know what you are doing, this is likely wrong. :warning:",
        );
        expect(loggerInfo).toHaveBeenCalledWith("Starting Persona Core for a new world, welcome aboard :rocket:");
    });
